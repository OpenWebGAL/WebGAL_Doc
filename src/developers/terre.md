...
AI 改变了软件开发的方式，在大幅提高软件开发效率的同时，也带来了诸多挑战。在 UI 编写上，使用 AI 大范围编写 UI 极易出现被称为 “AI Slop” 的问题，AI 编写的 UI 常常不符合人类使用习惯，时而会出现无意义的说明文本，AI 编写的 UI 在布局方面也常常出现溢出、错位等问题。AI 编写代码时也极易为了快速修补错误写出大量冗余代码或“防御性编程”代码。AI 也难以写出更加精简、易于阅读和维护的代码。

WebGAL 项目，对使用 AI 制定了一些规范，在编写代码时，请遵守这些规范：

1. 人类设计架构和实现方案，AI 编写。我们不接受直接向 AI 说明目标，但人类不参与设计的编码方式。在 AI 编写代码后，需要人工审阅代码，并测试相关模块后，才可以提交 PR。

2. Issue, PR 的文本不允许使用 AI 生成。哪怕不够详细，只需要给出足够的信息定位问题，或说明自己的用意即可。太过于详细的 AI 生成说明反而不容易抓住重点，聚焦于真正重要的信息。

3. 对于 UI，应当由人工完成原型设计，AI 只能用于实现。AI 直接根据功能生成的 UI，在我们过去数个版本迭代的经验中，被认为是远远不符合预期的。
