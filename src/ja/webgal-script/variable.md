...
; battle.txt
ナレーション:{enemy} に遭遇した。残り HP は {hp}。;
```

ローカル変数はその一回のシーン呼び出しに属します。セーブデータと一緒に保存・復元されますが、呼び出しが終わると消滅し、呼び出し元には残りません。そのため、同じシーンを異なる引数で何度でも再利用できます。
...
変数を読み取るとき、WebGAL は次の順に検索し、最初に見つかったものを使います。

1. 現在のシーンのローカル変数
2. 通常変数
3. 永続（グローバル）変数

つまりローカル変数は同名の通常変数を**隠します**。ローカル変数 `hp` と通常変数 `hp` が同時に存在する場合、`{hp}` は必ずローカルの方を読み、`setVar:hp=1;`（`-local` なし）は通常変数の方に書き込むため、書いた値を読み出せなくなります。引数の名前は通常変数と区別できるように付けてください。
...
呼び出されたシーンが `return` を実行せずに終わった場合は、空文字列が書き戻されます。
